...
then
    ...
    setarch "$(run_getpersonality)" -- sudo -E chroot "${CHROOT_NAME}" /bin/bash << '    EOF'
        ...
        echo "EMERGE_DEFAULT_OPTS=\"--autounmask --autounmask-continue --autounmask-backtrack=y --complete-graph --deep --update --newuse --usepkg --getbinpkg --backtrack=300 --usepkg-exclude dev-perl/Mozilla-CA\"" >> /etc/portage/make.conf
